...
static struct sock_filter backbone_filter[] = {
...
#define ASM_BPF_LD(_w, _m, _offs)    { BPF_LD | (_w) | (_m), 0, 0, (_offs) }
	...
	/* OUI Must be Westermo, protocol must be backbone. */
...
static int lag_port_setup_io(struct lag_port *p)
{
	...
	fprog.filter = backbone_filter;
	fprog.len = sizeof(backbone_filter) / sizeof(backbone_filter[0]);
...
static void lag_ev_tx_timer(struct ev_loop *loop, ev_timer *timer, int revents)
{
	...
	if (lag->conf->backbone_id(&lag->tx.pkt.id)) {
...
static void lag_setup_tx(struct lag *lag)
{
	...
	struct backbone_hello *hello = &lag->tx.pkt;
	int i;
