...
static inline int usbboot_id_cmp(const struct usbboot_id *a,
				  const struct usbboot_id *b)
{
...
struct usbboot_hello {
	uint8_t  slow_subtype;

	uint8_t  oui[3];
	uint8_t  protocol;

	struct usbboot_id id;
} __attribute__ (( packed ));
...
struct lag_config {
	...
	struct ev_loop *loop;
	int  (*usbboot_id)(struct usbboot_id *id);
	bool (*update)(void);
};
