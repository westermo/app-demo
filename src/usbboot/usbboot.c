...
struct usbboot_id current_id = { 0 };
...
static int set_station(void)
{
	...
	for (ifa = ifaddrs; ifa != NULL; ifa = ifa->ifa_next) {
		...
		/* Use the usbboot bridge's MAC to identify this
		 * node. */
		if (strcmp(ifa->ifa_name, "usbboot"))
			continue;
...
static bool set_id(const struct usbboot_id *new)
{
	...
	snprintf(netchid, sizeof(netchid), "/usr/lib/usbboot/net-chid.sh %u",
		 current_id.id);
...
static int usbboot_cb_id(struct usbboot_id *id)
{
	*id = current_id;
	return 0;
}

static bool usbboot_cb_update(void)
{
	struct usbboot_id new = { 0 }, *id;
	int d, p;
...
static int usbboot_run(struct ev_loop *loop)
{
	char netup[0x40];
	int err, i;
	...
	for (i = 0; i < 2; i++) {
		...
		lag_confs[i].usbboot_id = usbboot_cb_id;
		lag_confs[i].update = usbboot_cb_update;
	 ...
	 * setting up the usbboot. */
	snprintf(netup, sizeof(netup), "/usr/lib/usbboot/net-up.sh %u", vid);
	err = system(netup);
	if (err) {
		ERR("Unable to create usbboot bridge: %d\n", err);
		return err;
	}
...
int main(int argc, char **argv)
{
	...
	if (config_parse("/etc/usbboot.json"))
		return 1;
